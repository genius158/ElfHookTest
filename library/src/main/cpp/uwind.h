...
#define SAMPLE_LOG_PRIORITY   ANDROID_LOG_ERROR
    ...
    if (g_solution == SAMPLE_SOLUTION_FP || g_solution == SAMPLE_SOLUTION_EH) {
        if (!g_signal_interrupted) {
            if (g_solution == SAMPLE_SOLUTION_FP) {
                ...
                // EH local unwind
                size_t frames_sz = xunwind_eh_unwind(g_frames,
                                                     sizeof(g_frames) / sizeof(g_frames[0]),
...
#pragma clang diagnostic pop
        ...
        if (!(WIFEXITED(status)) || 0 != WEXITSTATUS(status)) {
            if (WIFEXITED(status) && 0 != WEXITSTATUS(status))
                SAMPLE_LOG("child terminated normally with non-zero exit status: %d",
                           WEXITSTATUS(status));
...
static void sample_sigsegv_handler(int signum, siginfo_t *siginfo, void *context) {
    ...
    pid_t tid = gettid();
    if (__atomic_compare_exchange_n(&g_cur_tid, &tid, 0, true, __ATOMIC_SEQ_CST,
                                    __ATOMIC_SEQ_CST)) {
...
static void sample_test_fp(jboolean with_context, jboolean signal_interrupted) {
    sample_test(SAMPLE_SOLUTION_FP, JNI_FALSE, with_context, signal_interrupted);
}

static void
sample_test_eh(JNIEnv *env, jobject thiz, jboolean with_context, jboolean signal_interrupted) {
    (void) env, (void) thiz;
